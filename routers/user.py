...
@router.get(
    ...
    description="마이페이지 '스탬프' 탭 데이터입니다. **미달성 칸까지 전부** 담아 내려주므로 "
                ...
                "- 단 `SCENERY_PHOTOS`(사진)와 `FIVE_REELS`(영상)는 **다녀온 여행이 아니어도** "
                "셉니다. 여행이 끝나서가 아니라 올리는 행동으로 켜지는 칸이라, 종료일을 "
                "기다리면 방금 올린 것이 0으로 보이기 때문입니다.\n"
                "- `SCENERY_PHOTOS`는 `POST /api/travels/{travel_idx}/images`로 올린 사진 "
                "장수입니다. 일정 매핑(`schedule_idx`) 여부와 무관하게 여행에 올린 사진이면 "
                "전부 세고, 지운 사진은 뺍니다.\n"
                "- **이미 딴 칸(`achieved: true`)의 `progress`는 항상 `goal`입니다.** 나중에 "
                "사진·여행을 지워 실제 수가 줄어도 그렇습니다 — 한 번 딴 스탬프는 조건이 "
                "어긋나도 유지되므로, 잠긴 칸에서만 `progress`를 진행도로 읽으세요. "
                "(예: 사진 11장 → 획득 → 3장 삭제 → 실제 8장이지만 응답은 `10/10`)\n"
