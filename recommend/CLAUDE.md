...
시골 도착역(예: 고래불역=영덕 병곡)은 선택 테마의 TourAPI 등록 관광지가 **0~2건**이다. 그러면
`build_courses`가 빈 목록을 내고 `_itineraries_from`의 마지막 폴백이 걸려 **"기차만 있는 카드" 3장**이
나온다(자동 경유가 섞이면 "중간에 아무데나 들렀다 가는" 카드로 보인다 — 경유역 테마 점수가 전부 0으로
동점이라 정렬이 이동시간 순으로 무너지기 때문). 이를 막기 위해 조건을 단계적으로 푼다.

- **사다리 — `utils/tour_place.live_places_relaxed`** (`_PLACE_FALLBACK_STEPS`)
  `(20km, 테마유지) → (35km, 유지) → (50km, 유지) → (20km, 완화) → (50km, 완화)`.
  `min_count`를 채우는 **첫 단계에서 멈춘다**(도심은 첫 단계에서 끝나 추가 호출 0). 끝까지 못 채우면
  가장 많이 나온 단계를 쓰고, 동수면 앞 단계(덜 푼 조건)를 유지한다. `seed`로 이미 조회해 둔 결과를
  넘기면 첫 단계를 건너뛴다.
  - **반경 먼저, 테마는 마지막**: 테마는 사용자가 명시적으로 고른 조건이다.
  - **완화 첫 단계가 기본 반경으로 되돌아오는 이유**: 조건을 다 풀 바에는 '멀리 있는 딱 맞는 곳'보다
    '가까운 다른 테마'가 낫다(도착역에 내려서 실제로 갈 수 있어야 한다).
  - **50km 상한**: `locationBasedList2`는 더 넓은 반경도 받지만(실측), 더 넓히면 "역은 고래불인데 일정은
    안동"이 되어 Phase B가 없앤 '도착역-코스 어긋남'이 되돌아온다.
- **적용 지점 — `services/recommend_service`**: `_scan_places`(목적지, 기준 `_MIN_PLACES_PER_DAY × 일수`)와
  `_course_for_overnight`(숙박경유의 경유 도시)**만**. 기준이 `3 × 일수`인 건 `build_courses`가 상위 후보를
  3버킷으로 쪼개기 때문 — 그보다 적으면 플랜 3장이 비거나 서로 같아진다.
- ⚠️ **완화한 장소는 점수화도 완화해야 한다.** `scoring.score_places`는 선택 테마와 하나도 안 겹치는 곳을
  **0점으로 버리므로**, 완화해 받아온 장소를 원래 테마로 점수화하면 전부 탈락해 폴백이 무효가 된다.
  `_itineraries_from(themes_relaxed=True)` → `_prepare_scored(themes=[])`가 그 연결이다.
- ⚠️ **폴백을 태우지 않는 곳**(의도적):
  - **도착지 자동선택 Phase B**(`_recommend_auto_dest`의 `place_cache`): 완화한 결과로 재점수하면
    '볼 게 없어서 밀렸어야 할 후보'가 도로 올라와 도착지 순위가 뒤틀린다. 코스용으로만 넓힌다.
  - **경유역 스캔**(`_enrich_stopovers`, 3km): 그 건수가 곧 '테마 관련도' 점수라 넓히면 순위 신호가 깨진다.
- **무엇을 풀었는지 밝힌다**: `PlaceScan`(`widened`/`themes_relaxed`) → `_scan_note`가 문구를 만들어
  `DestinationPlan.note`에 싣는다(`_join_notes`로 route note와 ` / ` 결합). 조용히 풀면 "바다 골랐는데 왜
  서원이 뜨지"가 된다. **앱은 이 note를 반드시 노출해야 한다.**
