...
- **로그아웃하면 그 사용자의 기기 토큰을 전부 푼다**(`auth_service.logout` → `fcm_token_dao.soft_delete_by_user`). 로그아웃한 기기로 푸시가 계속 가면 안 되는데(본문에 여행 제목·닉네임·좌석번호가 실려, 기기를 넘기거나 공유하면 다음 사람이 이전 사용자의 알림을 읽는다) **서버는 어느 기기가 로그아웃하는지 모른다** — access 토큰에 세션 식별자가 없고(`core/security.py:_create_token`) refresh 토큰의 `jti`는 기기가 아니라 발급 회차라 회전마다 바뀐다. 그래서 요청 스키마를 안 건드리는 대신 과하게 지우는 쪽을 택했다. **대가**: 다중 기기 사용자가 한 곳에서 로그아웃하면 다른 기기도 앱을 다시 열어 재등록할 때까지 푸시가 멈춘다(놓친 건 알림 화면 이력에 남는다). 정확히 하나만 끊고 싶으면 로그아웃 요청에 `fcm_token`을 선택 항목으로 받되 **삭제 조건에 소유자를 반드시 함께 걸어라** — 이 엔드포인트엔 access 인증이 없어 토큰 문자열만으로 남의 기기 푸시를 끊는 수단이 된다. `logout-all`·탈퇴도 같은 함수로 전부 지운다(기기 분실 대응인데 세션만 끊으면 푸시가 그대로 간다).
- **토큰 소유자 교체는 막지 않고 로그만 남긴다**(`fcm_service.register_token`). FCM 토큰엔 소유를 증명할 수단이 없어 문자열만 알면 남의 기기를 자기 계정에 붙일 수 있다(피해자는 자기 알림을 못 받고 그 기기엔 공격자 알림이 뜬다). 그래도 거절하면 **세션 만료로 로그아웃을 못 거친 기기에 다른 계정이 로그인하는 정상 경로**가 아무 신호 없이 영구히 푸시를 잃는다. 그래서 살아 있는(`deleted_at IS NULL`) 남의 토큰을 가져갈 때만 WARNING을 남긴다 — 정상 인계는 로그아웃을 거쳐 조용하다.
- **개발용 발송 API는 없다**: `POST /api/fcm/test-send`는 인증 없이 본문의 `user_idx`로 임의 사용자에게 임의 문구를 쏠 수 있어 삭제했다. 되살릴 거면 `get_current_user`를 걸고 대상은 `current_user` 고정 — 대상을 본문으로 받는 형태로 두지 마라.
