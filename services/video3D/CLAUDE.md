...
Seasonal themes (`default`/`spring`(alias `sakura`)/`summer`/`autumn`/`winter`) live in `map_themes.js` as `window.MapThemes` — the single registry `THEMES` plus `applyTheme(map, name, opts)`. Each theme is `{ lightPreset?, fog?, colorGrade?, snow?, routeLine?, trees? }`; **missing fields are restored to defaults on every apply**, so switching to `default` fully reverts (snow removed via `setSnow(null)`, identity LUT, default fog, original line colors). `colorGrade` bakes a 32³ LUT (`greenToAmber` for autumn foliage, `greenToPink` for sakura pastel). `trees` plants deterministic (seeded PRNG) cherry-tree symbol points alongside the route — spacing auto-widens so `maxCount` spreads over the whole route; layer hidden below `minZoom` 10 (they'd smear into a band). Selection flows: builder `#theme` select → `POST /api/render` `theme` form field → `--theme` CLI flag (local and `modal_render.py`) → `window.MAP_THEME` injection → `applyMapTheme()` in `map_animation.js`, which must run **after** `addRouteLayers()` (it styles `progress-route-casing`, a width-0 casing layer under the progress line, and inserts trees below `route-marker-halo`). The builder preview (`builder.html`, GL JS ≥3.9 required for `setSnow`) applies themes live via the same module (served at `GET /map_themes.js`) and re-plants trees on route edits with `MapThemes.refreshTrees`. To add a theme: add one `THEMES` entry, then extend `--theme` choices in `render_video.py`, `ALLOWED_THEMES` in `server.py`, and the builder dropdown.

### TRAILER intro (`intro_video.py`, opt-in via `--intro`)
A text-mask zoom intro (~3.6s) prepended to the finished MP4 as a **post-processing step** — Pillow + FFmpeg only, no browser/GPU, so cost is a flat ~5s regardless of video length. "TRAILER" (heavy font, `FONT_CANDIDATES`: Impact → Arial Black → … → Linux fallbacks; Modal container has no Impact) acts as a mask: the main video (from `BG_START_SECONDS`, pulled forward for short videos) shows through the glyphs, the text is **redrawn per frame** at exponentially growing size (never rescaled — stays sharp) zooming into the middle glyph `I`, fades to black, then fades into the main video's first frame. Because the intro's last frame equals the main's first frame and the clip is encoded with **identical codec params** (libx264 same preset/crf, `profile high`, `yuv420p`, `-video_track_timescale` **probed from the main video** — a mismatched timescale silently stretches the main's playback speed after concat, plus a matching silent AAC track iff the main has BGM — probed via ffprobe), the join is a stream-copy concat (`-c copy`): the main video is never re-encoded. Flow: builder `#intro` checkbox → `intro` form field → `--intro` flag (local & `modal_render.py`) → `prepend_intro()` in `run()`, which must run **after** `mux_bgm_into_video` (audio track must exist when probed) and is skipped for `--benchmark-frames`. Note: the intro is additive — it is not counted in the 60s `max_video_seconds` cap.
