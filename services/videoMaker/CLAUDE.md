...
- `GET /api/videos/bgm` lists tracks (`CommonResponse` envelope; `_bgm_display_name()` parses Pixabay attribution filenames); `GET /api/videos/bgm/preview?file=` streams one; `GET /api/videos/assets/map_themes.js` serves the shared theme module; `GET /api/videos/reels/{reels_idx}/download` streams the requester's own finished mp4 (JWT required; the old filename-based `/api/videos/output/{name}` endpoint was removed).
- `POST /api/videos/render` (multipart form, `CommonResponse[VideoRenderResponse]`) saves uploaded photos to `assets/uploads/<job>/`, writes a `travel_data.json` (with the `bgm` field), then dispatches on the `engine` form field:
  - `engine=local` → subprocess `render_video.py` using the **trailer3d python from `[videomaker] python` in `properties_dev.ini`** (falls back to `sys.executable`); non-quick renders default to `--quality-fast` (full-res JPEG q95 — visually identical to lossless in the final mp4, much faster); output parsed from `"출력 예정 파일:"`.
  - `engine=modal` → subprocess `python modal_call.py --travel-data <rel> ...` which **calls the DEPLOYED Modal function** (`modal deploy modal_render.py` once beforehand — bakes image + static assets/bgm so per-render setup drops from minutes to container boot). Photos are sent as bytes per call; mode is always quality-fast; container logs stream through `modal.enable_output()` so the `[perf:frame]` progress parsing works; output parsed from modal_call's `"저장 위치:"` line. Changing the Mapbox token or theme/render code requires `modal deploy` again.
...
- `clip` — a short video mixed into the same `photos` list (told apart by extension, `CLIP_EXTENSIONS`). Only the first `CLIP_MAX_SECONDS` (5s) are used; duration comes from ffprobe (`clip_play_seconds`, cached). `emit_clip_segment_frames` decodes it with ffmpeg to raw RGB (fps-normalized, crop-to-fill — no blur backdrop, too slow per frame) and fades over the held map frame **while playing** (fades overlap the clip, so the segment is exactly the clip length). Audio is dropped; BGM is muxed over the whole video afterwards so it never breaks. Frame count is fixed by the timeline — short decodes are padded with the previous frame.
- **Place label**: photo and clip frames get a place name drawn at the top — per photo from the optional top-level `photoLabels` map (`{photo path as written in photos: name}`, `""` = hide), else the stop's `name` (`MediaPoint.label_of`). The travel render writes it because stops within 1km are merged, so one stop can hold photos from several schedules (경복궁 → 국립민속박물관); `modal_call.split_travel_data` copies it into every chunk. Drawn by `place_label_overlay` (dark gradient band + white text, shrinks to fit width, cached per name) in the **same repo font as the cover/intro** (`find_label_font()` → `assets/fonts/Handwriting.ttf`; see the fonts gotcha below). It's baked into the photo/frame before the map crossfade, so it fades with the media. An empty name draws nothing — and empty names stay empty (no `Track N` placeholder, in `validate_travel_data` or `labelForTrackIndex`). The service fills unnamed stops from coordinates (`video_service._name_stops` → `kakao_local.place_name_of`: nearest AT4 attraction within 300m skipping route/street categories like 도보여행·테마거리·먹자골목·…길, with "경복궁 연생전" lifted to "경복궁"; else "구 동"), leaving them empty if lookup fails. It's a heuristic — a closer minor spot can win, and temples aren't AT4.
...
- **Two font lookups, don't mix them up.** `find_label_font()` (place labels on photos/clips) takes the **repo font** `assets/fonts/Handwriting.ttf` first — the same file the cover/intro uses (`utils/cover_image.py:TITLE_FONT`) so the first scene and the body of the reel are one typeface. The two constants are separate on purpose (this file runs alone in the Modal container and can't import `utils`), so **change both together** — `tests/test_label_font.py` guards that. The repo font lives outside this directory, so `modal_render.py` uploads it separately to `/app/assets/fonts`; **`modal deploy` is required** after touching the font or that path, and if it's missing the container silently falls back to its Noto gothic (no error, just different-looking labels). `LABEL_SIZE` (0.070) is higher than the old bold gothic's 0.058 because a handwriting face sits ~0.83× smaller at the same pt.
- **`find_font()` is Windows paths** (`C:/Windows/Fonts/malgun.ttf`, …) and is only the fallback chain plus the auto-generated destination photo, whose Korean labels depend on `malgun`; on a non-Windows host it falls back to ASCII text and Pillow's default font.
